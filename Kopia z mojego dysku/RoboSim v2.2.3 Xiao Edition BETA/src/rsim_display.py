...
version = "V2.2.3 Xiao Edition BETA"
...
class PrefsDialog(gui.Dialog):
    def __init__(self, noise_model, noise_value, rand_under, motor_noise1):
        ...
        tt.td(gui.Spacer(width=8,height=8))
        tt.tr()
        tt.td(gui.Switch(name="motorn", value=motor_noise1))
        tt.td(gui.Label(" Motor Encoder Noise")) 
        tt.tr()
        tt.td(gui.Spacer(width=8,height=8))
        tt.tr()
        t.td(tt,colspan=2)
...
class Display():
    def __init__(self, zoom):
        ...
        self.menus = gui.Menus([
            ...
            ('File/Kill Connection', self.kill_connection,None),
...
#            ('Controls/Keep Centered', self.foo, None),
        ...
        self.speed_up = 1
        self.speed_up_scale = sgc.Scale(pos=(800, self.currentHeight - 60), min=1, max=4, label="Speed Up", label_col=(0,0,0), show_value=0, label_side="top")
    ...
    def kill_connection(self, param):
        #kill the connection between the robot and the cmd for windows (-.-') users
        self.want_to_kill = True
    ...
    def sim_prefs(self, param):
        self.prefs_d = PrefsDialog(self.noise_model, self.noise_value, 
                                   self.rand_under, self.world.robot.motor_noise)
    ...
    def handle_prefs_dialog_close(self, dlg):
        ...
        for k,v in dlg.value.items():
            ...
            if k=='motorn':
                motor_n = v
        self.set_noise_model(noise_model, noise_value, rand_under, motor_n)
        self.prefs_d.close()
    ...
    def set_motor_noise(self):
        #self.set_active();
        if self.motor_noise_switch.state:
            self.world.robot.motor_noise = 1
        else:
            self.world.robot.motor_noise = 0
    ...
    def set_noise_model(self, noise_model, noise_value, rand_under, motor_n):
        self.set_active();
        self.noise_model = noise_model
        self.noise_value = noise_value
        self.rand_under = rand_under
        self.world.robot.motor_noise = motor_n

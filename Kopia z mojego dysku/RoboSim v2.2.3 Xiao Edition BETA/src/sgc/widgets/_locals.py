...
class Font():
    ...
    def set_fonts(cls, fonts={}):
        ...
        for font in fonts:
            ...
            cls._fonts[font].replace(cls._create_font(fonts[font], 14))

        if not cls._fonts["widget"]:
            cls._fonts["widget"].replace(cls._create_font("Arial", 14))
        if not cls._fonts["title"]:
            name = fonts["widget"] if ("widget" in fonts) else "Arial"
            cls._fonts["title"].replace(cls._create_font(name, 24))
        if not cls._fonts["mono"]:
            cls._fonts["mono"].replace(cls._create_font(
                "Ubuntu Mono, FreeMono, Monospace", 14))

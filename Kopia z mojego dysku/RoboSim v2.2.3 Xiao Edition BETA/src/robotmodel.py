...
class RobotModel(BaseObj):
    ...
    def update_position(self):
        ...
        # check if we want to use random values for motors as well or not
        if (self.motor_noise == 1):
            threshold = 0.95     # leave this below 1
            self.encoder[L] += dl*uniform(threshold, 1/threshold)
            self.encoder[R] += dr*uniform(threshold, 1/threshold)
        else:
            self.encoder[L] += dl
            self.encoder[R] += dr

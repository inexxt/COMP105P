...
float start_rangle = 55, start_langle = 20; 
...
// 	printf("test 4 : %d\n", two_vectors_cross(4,5,1,2,3,1));
	...
	while(var)
	{
		...
		if(leftFsensor() < 40)
		{
			...
			acc += Ks*exp(r_diff- 40 - Ks_d*fabs(l_diff - pl_diff)); //differentiating l_diff, because it's more reliable
		}
		else
		{
			rspeed = 70;

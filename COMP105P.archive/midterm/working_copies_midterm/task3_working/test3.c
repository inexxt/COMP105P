...
void initialize_list(double l, double r)
{
...
{	
	...
	push(head, wheelDtoCm(ldist-lpreviousdist), wheelDtoCm(rdist - rpreviousdist));
...
// 		if(counter == 100) var = 0;
	...
	set_motors(0,0);
// 	print_list(head);
	printf("Angle %.2f wV %.2f wH %.2f\n", computeAngleList(head), computeWayList(head, VERTICAL), computeWayList(head, HORIZONTAL));	
}
